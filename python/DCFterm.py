...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        "--growth_rate",
        type=float,
        help="Perpetuity growth rate as a decimal (e.g., 0.02 for 2%%), optional if using interactive mode.",
    )
    parser.add_argument(
        "-i",
        "--interactive",
...
def main():
    ...
    # Plotting
    years = list(range(1, len(projected_cash_flows) + 1))  # Projected cash flows years
    terminal_year = len(projected_cash_flows) + 1  # Year for the terminal value

    plt.figure(figsize=(10, 5))
    plt.plot(years, projected_cash_flows, "-o", label="Projected Cash Flows")
    # Plot the terminal value as a single point
    plt.plot(
        [terminal_year], [terminal_value_pv], "ro", label="Terminal Value"
    )  # Corrected to match dimensions
